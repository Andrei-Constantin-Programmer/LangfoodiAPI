...
public class UnblockConnectionHandlerTests
{
    ...
    private readonly UnblockConnectionHandler _unblockConnectionHandlerSUT;

    public UnblockConnectionHandlerTests()
    {
        ...
        _unblockConnectionHandlerSUT = new(
    ...
    public async Task Handle_WhenUserDoesNotExist_ThrowUserNotFoundException()
    {
        ...
        UnblockConnectionCommand command = new("nonExistantUserId", "conn1");

        // When
        var testAction = async () => await _unblockConnectionHandlerSUT.Handle(command, CancellationToken.None);
    ...
    public async Task Handle_WhenConnectionDoesNotExist_ThrowConnectionNotFoundException()
    {
        ...
        UnblockConnectionCommand command = new(user.Account.Id, "conn1");
    ...
    [Trait(Traits.DOMAIN, Traits.Domains.USER)]
    [Trait(Traits.DOMAIN, Traits.Modules.APPLICATION)]
    public async Task Handle_WhenUnblockIsSuccessful_UpdateUser()
    {
        // Given
        ...
        Connection otherConnection = new("conn2", user1.Account, user3.Account, ConnectionStatus.Connected);

        UnblockConnectionCommand command = new(user1.Account.Id, connection.ConnectionId);

        user1.Account.BlockConnection(connection.ConnectionId);
        user1.Account.BlockConnection(otherConnection.ConnectionId);

        // When
        await _unblockConnectionHandlerSUT.Handle(command, CancellationToken.None);
        ...
        user1.Account.BlockedConnectionIds.Should().NotContain(connection.ConnectionId);
        user1.Account.BlockedConnectionIds.Should().Contain(otherConnection.ConnectionId);
    ...
    [Trait(Traits.DOMAIN, Traits.Domains.USER)]
    [Trait(Traits.DOMAIN, Traits.Modules.APPLICATION)]
    public async Task Handle_WhenUnblockIsUnsuccessful_DoNotUpdateUser()
    {
        // Given
        TestUserCredentials user1 = new()
        {
